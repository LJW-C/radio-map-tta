...
class AOTBlock(nn.Module):
    def __init__(self, dim, rates):
        ...
        # Debugging: Print the type of each rate
        ...
        for i, rate in enumerate(rates):
            self.__setattr__(
                ...
                nn.Sequential(
                    nn.ReflectionPad2d(rate),  # Ensure this is an integer
                    nn.Conv2d(dim, dim // 4, 3, padding=0, dilation=rate),
                    nn.ReLU(True)))

        self.fuse = nn.Sequential(
            nn.ReflectionPad2d(1),  # Ensure this is an integer
    ...
    def augment(self, x):
        padding_value = (1, 1, 1, 1)  # Example: Padding value for all sides
        print(f"Padding value: {padding_value}")  # Ensure this prints an integer or tuple of integers
        x = F.pad(x, padding_value, mode='reflect')
        return x
...
class Generator(BaseNetwork):
    def __init__(self, block_num=1, rates="1+2+4+8"):
        super(Generator, self).__init__()

        # 将 rates 转换为整数列表
        self.rates = [int(rate) for rate in rates.split('+')]
        ...
        self.middle1 = nn.Sequential(*[AOTBlock(64, self.rates) for _ in range(block_num)])  # channel:64

        self.conv2 = convrelu(64, 128, 4, 2, 1)  # size:128
        self.att2 = cbam_block(128)
        self.middle2 = nn.Sequential(*[AOTBlock(128, self.rates) for _ in range(block_num)])  # channel:128

        self.conv3 = convrelu(128, 256, 4, 2, 1)  # size:64
        self.att3 = cbam_block(256)
        self.middle3 = nn.Sequential(*[AOTBlock(256, self.rates) for _ in range(block_num)])  # channel:256

        self.conv4 = convrelu(256, 512, 4, 2, 1)  # size:32
        self.att4 = cbam_block(512)
        self.middle4 = nn.Sequential(*[AOTBlock(512, self.rates) for _ in range(block_num)])  # channel:512
        ...
        self.middle5 = nn.Sequential(*[AOTBlock(64, self.rates) for _ in range(block_num)])  # channel:64

        self.conv6 = convrelu(64, 128, 4, 2, 1)  # size:128
        self.att6 = cbam_block(128)
        self.middle6 = nn.Sequential(*[AOTBlock(128, self.rates) for _ in range(block_num)])  # channel:128

        self.conv7 = convrelu(128, 256, 4, 2, 1)  # size:64
        self.att7 = cbam_block(256)
        self.middle7 = nn.Sequential(*[AOTBlock(256, self.rates) for _ in range(block_num)])  # channel:256

        self.conv8 = convrelu(256, 512, 4, 2, 1)  # size:32
        self.att8 = cbam_block(512)
        self.middle8 = nn.Sequential(*[AOTBlock(512, self.rates) for _ in range(block_num)])  # channel:512
    ...
    def forward(self, build, antenna, return_features=False):
    # def forward(self, samples, mask):
        # 1
        x1 = torch.cat([build, antenna], dim=1)

    # 检查输入图像的通道数

        # 2
        # samples = F.interpolate(samples, size=(build.size(2), build.size(3)), mode='bilinear', align_corners=False)
        # x1 = torch.cat([build, antenna, samples], dim=1)
        # 3
        # x = torch.cat([samples, mask], dim=1)

        # PHASE 1

        # encoder
        layer_conv1 = self.conv1(x1)
