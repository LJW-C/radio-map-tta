...
class RadioWNet(nn.Module):
    def __init__(self, inputs=2, phase="firstU"):
        ...
        self.inputs = inputs  # 从参数获取输入通道数
        self.phase = phase    # 从参数获取 phase
        ...
        self.Wlayer00 = convrelu(self.inputs+1, 20, 3, 1,1)
        ...
        self.Wconv_up00 = convrelu(20+20+self.inputs+1, 20, 5, 2,1)
        self.Wconv_up000 = convrelu(20+self.inputs+1, 1, 5, 2,1)

    def forward(self, build, antenna, return_features=False):

        # 1
        input0 = torch.cat([build, antenna], dim=1)
        ...
        if self.phase == "firstU":
            ...
            # 第二部分网络
            Winput = torch.cat([output1, input0], dim=1)
        ...
        if return_features:
            return layer4u, output1 # 返回中间特征 f6 和输出 y
        else:
            return output1
